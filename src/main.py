...
def solveEveryInstance(solname="cur_solution.zip"):
    ...
    debugUID = "simple-polygon-exterior-20_10_868921c7"#"simple-polygon-exterior-20_20_0f96fb2c"#"simple-polygon-exterior-20_10_8c4306da"#point-set_10_13860916"
...
def mergeEveryInstance():
    ...
    for instance in idb:
        if i!=0:
            ...
            bestSolution = [sol[i] for sol in solutions][10]#best[i]
            otherSolutions = [sol[i] for sol in solutions]
            #bestTri = Triangulation(instance,False,0,axs,True)#triangulationFromSolution(instance,bestSolution,axs=axs)
            bestTri = triangulationFromSolution(instance,bestSolution,axs=axs)
...
def pooledMergeWorker(index):
    ...
    for cIdx in range(len(currentInstance)):
        if currentInstance[cIdx] == -1:
            ...
            break
    if myIdx == -1:
        logging.error(f"failed to obtain valid ID...")
        assert(False)
    #assert(myIdx != -1)
    ...
    try:
        ...
        for summary in summaryFolder.iterdir():
            try:
                ...
                logging.error(f"{multiprocessing.current_process()} ({myIdx}): working on instanceId {instanceIdx} of name {instance.instance_uid} -> Failed to load {summary.name}")
...
def mergerPool():
    ...
    with Pool(initializer=init_real_pool_processes,initargs=(lock,returner,None,best,times,instanceNos,numInstances,[ins for ins in idb])) as pool:
        ...
        while not result.ready():
            ...
            numUpdates = 0
            lock.acquire()
            solLoc = filepath.parent.parent / "out_merged_summaries"
